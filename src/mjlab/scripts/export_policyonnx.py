  ...
  while changed:
    ...
    for tensor_name in list(needed_tensors):
      if not tensor_name:
        continue
      if (
        tensor_name in graph_input_names
        or tensor_name in initializer_names
        or tensor_name in sparse_initializer_names
      ):
        continue
...
def prune_model(model, keep_input: str, keep_output: str, remove_input: str):
  ...
  if keep_input not in graph_input_names:
    raise ValueError(
      f"找不到要保留的输入 {keep_input!r}。当前输入: {sorted(graph_input_names)}"
    )
  if keep_output not in graph_output_names:
    raise ValueError(
      f"找不到要保留的输出 {keep_output!r}。当前输出: {sorted(graph_output_names)}"
    )
  ...
  kept_sparse_initializers = [
    s for s in sparse_inits if s.values.name in needed_tensors
  ]
...
def parse_args():
  parser = argparse.ArgumentParser(
    description="删除 time_step 及其依赖输出，只保留 obs -> actions 子图"
  )
  ...
  parser.add_argument(
    "--keep-output", default="actions", help="要保留的输出名，默认 actions"
  )
  parser.add_argument(
    "--remove-input", default="time_step", help="要删除的输入名，默认 time_step"
  )
